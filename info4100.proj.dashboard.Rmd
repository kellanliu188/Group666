...
student_dash = tabItem(
  ...
  # Any visualization
  fluidRow(
    box(title = "Quiz Score compare to Average Score",
        plotOutput("inst.plot7", height = 250)),
    box(title = "Engagement Rate compare to Overall Engagement",
        plotOutput("inst.plot8", height = 250)),
    box(title = "Clicker Line Correctness Plot",
        plotOutput("inst.plot9", height = 250)),
    box(title = "Radar Map of Skill Score",
        plotOutput("inst.plot10", height = 250))
  )
)
...
engagement_data = as.data.frame(list(name = c("Session", "Student"), 
                                     rate = c(class_engagament, student_80_engagement)))

#use the data below to create two heapmap for attendance check, one for class, one for student 80
student_80$correct_quiz_clicker = student_80$CORRECT_Q_CLICKER / student_80$COMPLETED_Q_CLICKER
student_80$correct_quiz_clicker[is.nan(student_80$correct_quiz_clicker)] = 0
quiz_clicker_compare = merge(session_2017, student_80, by = 'SESSION_NUMBER') %>%
  select(SESSION_NUMBER,
         correct_quiz_clicker,
         Avg_Correct_Quiz_Clicker)
...
colnames(average_skills) = colnames(student_skills_80)
max_scores = c(5, 5, 5, 5, 5, 5, 30)
min_scores = c(0, 0, 0, 0, 0, 0, 0)

skill_data = rbind(min_scores, max_scores, average_skills, student_skills_80)
rownames(skill_data) <- c("min", "max", "Class Average", "Student 80")
...
server = function(input, output) {
  ...
  output$inst.info3 = renderInfoBox({
    infoBox(
      "Average Quiz Score",
      round(mean(quiz$QUIZ_SCORE, na.rm = TRUE), 1),
  ...
  output$inst.plot4 = renderPlot({
    ggplot(session_summary,
           aes(x = YEAR, y = SESSION_NUMBER, fill = Avg_Temperature)) +
      geom_tile() +
      scale_fill_gradient(low = "lightgreen", high = "darkgreen") +
      labs(title = "Heatmap of Avg_Temperature", x = "Year", y = "Session Number") +
      theme_minimal() + # Minimal theme
      scale_y_continuous(breaks = seq(
        from = min(session_summary$SESSION_NUMBER),
        to = max(session_summary$SESSION_NUMBER),
        by = 2
      ))
  })
  ...
  ## student plots
  # Line charts showing quiz scores over time compared to the class average.
  output$inst.plot7 = renderPlot({
    ggplot(quiz_data_comapre, aes(x = SESSION_NUMBER)) +
      geom_line(aes(y = QUIZ_SCORE, group = 1, colour = "Quiz Score")) +
      geom_line(aes(y = Avg_Quiz_Score, group = 1, colour = "Class Average")) +
      labs(title = "Quiz Scores Over Time Compared to Class Average",
           x = "Session Number",
           y = "Score") +
      scale_colour_manual("", 
                          breaks = c("Quiz Score", "Class Average"),
                          values = c("blue", "red")) +
      theme_minimal()
  })
  
  # bar graphs comparing the student engagement with the rest of the student
  output$inst.plot8 = renderPlot({
    ggplot(engagement_data, aes(x = name, y = rate, fill = name)) +
      geom_bar(stat = "identity") +
      labs(title = "Engagement Rate of Student 80 and Session", 
           x = "", 
           y = "Engagement Rate") +
      scale_fill_manual(values = c("blue", "red")) +
  ...
  output$inst.plot9 = renderPlot({
    ggplot(quiz_clicker_compare, aes(x = SESSION_NUMBER)) +
      geom_line(aes(y = correct_quiz_clicker, group = 1, colour = "Clicker")) +
      geom_line(aes(y = Avg_Correct_Quiz_Clicker, group = 1, colour = "Class Average")) +
      labs(title = "Clicker Over Time Compared to Class",
           x = "Session Number",
           y = "Clicker") +
      scale_colour_manual("", 
                          breaks = c("Clicker", "Class Average"),
                          values = c("blue", "red")) +
      theme_minimal()
  })
  ...
  # Radar charts to compare the student's self-reported skills against averages
  library(fmsb)
  output$inst.plot10 = renderPlot({
    # Plot the radar chart with corrected axis labels
    radarchart(
      skill_data,
      axistype = 1,
      pcol = c("blue", "red"),
      ...
      # Redefine axis labels to match the 0-5 range
      legend = rownames(skill_data)[3:4],
      cex = 0.8,
      title = "Average Skill Score compared to Student 80")
  })
  #######################################
  #######################################
...
dside = dashboardSidebar(sidebarMenu(
  ...
  menuItem("Student View",
           tabName = "student",
           icon = icon("th"))
))

# Here we set up the body of the dashboard
dbody = dashboardBody(tabItems(student_dash,
                               instructor_dash))
...
-   What do you like about it?
    -   I like the part being a real grad student to think about what kind of question we are interested about.
-   What would you change or add to it if you had more time?
    -   if we have more time, we could be more foucesd on data processing and think about more deep questions.
-   What was the biggest challenge you faced? How did you address it?
    -   The  Biggest challenge was to create the radar chart, since we do not have prior knowledge of that.

*Reflection for the teacher dashboard*

-   What do you like about it?
    -   I like the part of being able to think on the professor's side, to discuss what professors may want to know about a whole class.
-   What would you change or add to it if you had more time?
    -   If we have time, we would actually talk with a professor to get to know their actual thoughts.
-   What was the biggest challenge you faced? How did you address it?
    -   The biggest challenge was to create info box along with the chart. 

# Self-reflection

**Briefly summarize your experience on this homework. What was easy, what was hard, what did you learn?**

-   I think this is fun group project, to get know learning in a more depth.
...
-   I sometimes used to AI, mostly HiTA this time.
-   Upon using HiTA, I found it was really helpful for dealing with
    homework, because it can relate the contents for this week and
    referance what we were writing and familiar with the data set we
    were using. Unlike ChatGPT, it can be only used for general R
    debugging.
